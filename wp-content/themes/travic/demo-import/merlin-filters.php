 ...
 */
function travic_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'travic_unset_default_widgets_args' );
 ...
 */
function travic_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'travic_child_functions_php', 10, 2 );
 ...
 */
function travic_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'travic'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'travic_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'travic' ),
			'preview_url'              => 'http://commonsupport.net/newwp/travic/',
		),
	);
}

add_filter( 'merlin_import_files', 'travic_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function travic_after_import_setup() {
	...
	set_theme_mod(
		'nav_menu_locations', array(
			...
			'one_page_menu' => $one_page_menu->term_id,
		)
	);
...
add_action( 'merlin_after_all_import', 'travic_after_import_setup' );