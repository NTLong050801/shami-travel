...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'travic' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'TRAVIC/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( TRAVIC_NONCE ),
		);

		wp_localize_script( 'jquery', 'travic_data', $header_data );

		if ( travic_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', travic_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'TRAVIC/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "travic-{$name}", $style );
			} else {
				wp_enqueue_style( "travic-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = travic_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= travic_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'travic-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'travic-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$settings = get_theme_mod( 'travic' . '_options-mods' );

		if ( $custom_font = travic_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('travic_redux_custom_fonts_load', $custom_font );
