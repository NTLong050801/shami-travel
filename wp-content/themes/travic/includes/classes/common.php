...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => travic_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => travic_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => travic_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => travic_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => travic_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => travic_meta( 'banner_elementor_template', $page_id ),
				'banner'            => travic_set( travic_meta( 'banner_page_background', $page_id ), 'url' ),
				'banner_image'   	=> travic_set( travic_meta( 'banner_banner_image', $page_id ), 'url' ),
				'title'             => travic_meta( 'banner_banner_title', $page_id ),
				'text'             => travic_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => travic_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : travic_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : travic_the_title( $template ),
				'banner'        => travic_set( $bg, 'url' ),
				'banner_image'  => travic_set( $banner_image, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$title = travic_meta( 'banner_banner_title', $page_id );
		$text = travic_meta( 'banner_banner_text', $page_id );
		$banner_image = travic_meta( 'banner_banner_image', $page_id );
		$return = [
